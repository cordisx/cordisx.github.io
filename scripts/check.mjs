...
for (
  const [file, content, references] of [
    ...
    ['marketplace/index.html', marketplace, [
      ...
      'data-cordisx-animation="one-shot"',
      '../cordisx-mark-animated-dark.svg',
      'id="locale-toggle"',
      'id="theme-toggle"',
    ]],
    ['marketplace/plugin/index.html', pluginDetail, [
      '/site-shell.css',
...
if (!marketplaceScript.includes('/marketplace/plugin/?id=')) {
  throw new Error('marketplace cards must link to plugin details')
}
...
if (
  !homepagePreferences.includes('prefers-color-scheme: light')
  || !marketplaceScript.includes('prefers-color-scheme: light')
) {
...
if (
  !homepageStyles.includes(':root[data-theme="light"] .site-header') || !homepageStyles.includes('background: #e4e4e7')
) {
...
if (
  !homepage.includes('data-i18n-alt="showcaseWorkspaceAlt"')
  || !homepage.includes('data-i18n-aria-label="showcaseChoices"')
) {
  throw new Error('homepage showcase media and controls must be localized')
}
if (
  (homepage.match(/class="showcase-slide(?: is-active)?"/g) ?? []).length !== 6
  || !homepage.includes('<b data-showcase-index>01</b> / 06')
) {
...
if (
  !homepagePreferences.includes('showcaseVideo${language}${appearance}Webm')
  || !homepagePreferences.includes('showcaseVideo${language}${appearance}Mp4')
) {
...
if (
  homepage.includes('market-chat-plugin-state') || homepage.includes('partyActive') || homepage.includes('conceptOnly')
) {
  throw new Error('party demo must not include the removed preview state banner')
}
if (
  !homepageMarketplace.includes("partyInput.textContent = ''")
  || !homepageMarketplace.includes('partyButton.disabled = true')
) {
  throw new Error('party demo must clear the request and disable empty sends')
}
if (
  !marketplaceScript.includes('value.schemaVersion !== 3') || !marketplaceScript.includes('plugin.schemaVersion !== 3')
) {
...
if (!products.includes('https://github.com/cordisx/marketplace')) {
  throw new Error('products.yaml is missing marketplace')
}
...
const playbackProbe = selectPlaybackTimeline(
  Array.from(
    { length: 9 },
    (_, frame) => ({ frame, segment: frame < 7 ? 'work' : 'finish', sourceElapsedMs: frame * 500 }),
  ),
  { work: 5 },
  12,
)
if (
  playbackProbe.sourceFrameCount !== 9
...
for (
  const [label, pattern] of [
    ['MP4 faststart flag', /'-movflags',\s*'\+faststart'/u],
    ['scene pixel format argument', /'-pix_fmt',\s*scene\.output\.pixelFormat/u],
  ]
) {
  if (!pattern.test(aiPluginCapture)) throw new Error(`AI plugin capture is missing ${label}`)
}
if (
  !aiPluginWorkflow.includes('Do not replace the Codex shell')
  || !aiPluginWorkflow.includes('Dry run never reads authentication')
) {
...
for (const language of ['en', 'zh']) {
  for (const theme of ['dark', 'light']) {
    for (const extension of ['gif', 'json', 'mp4', 'plugin.tsx', 'webm']) {
      await access(
        new URL(`../assets/motion/cordisx-ai-plugin-demo-${language}-${theme}.${extension}`, import.meta.url),
      )
    }
